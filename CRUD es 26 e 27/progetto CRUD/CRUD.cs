...
namespace progetto_CRUD
{
	public partial class CRUD : Form
	{
		...
		//semplifica esercizio, fare solo quello chiesto
		//più prodotti uguali per riga
		...
		private void Shortcut(object sender, KeyEventArgs e)
		{
			...
			if (e.Control &&  e.Shift &&e.KeyCode == Keys.A && selis !=0)
			...
			if (control == true)
				switch (fun)
				{
					case 1: //create file
						...
						break;
					case 2: //open
						selis = int.Parse(SearchBox.Text);
						AddButton_Click(sender, e);
						break;
					case 3: //move
		...
		{//fun 4
			...
			for (int i = 1; i < selis; i++)
				line += int.Parse(lines[i]); //fa la somma di tutti i delimitatori
		...
		{// fun 7
			...
			while (ind < line + npro)
			{ //a tutti l'identificativo + 1, inizia cambiando quello appena stampato per ristamparlo
				seline++;
